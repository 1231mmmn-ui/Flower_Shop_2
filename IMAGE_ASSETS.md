...
| `customer-08` | おだやかで親しみやすい、ひ孫の誕生を楽しみにしている男性 |
...
この絵本挿絵タッチでは、20〜40代のあいだで顔立ちに強くしわ・年齢差を描き込むと、
かえって customer-05〜08（白髪など明確な加齢表現がある人物）とのシリーズ内の
統一感が崩れやすい。そのため、20〜40代の customer については

- 顔の描き分けよりも、髪型・服装・表情の落ち着き方・人物設定・台詞で年齢感を出す
- 「顔だけで実年齢が判別できるか」を過度に厳密に求めない
- 明確な加齢表現（白髪・深いしわなど）は、50代以降の customer に留める
...
> **（追記・腕は2枚に分けて持つ）**
>
> 表情2種のほかに、**その人の腕**を描いた2枚を持ちます。
>
> | ファイル | 用途 |
> |---|---|
> | `assets/customers/<id>-back-arms.png` | 肩から続く上腕・前腕・袖。800×800・透過。人物とまったく同じ枠 |
> | `assets/customers/<id>-front-hands.png` | 束を挟む指先・手首だけ。800×800・透過。同じ枠 |
>
...
>
> **最初は腕を1枚（前腕と手をまとめて描いたもの）で試しましたが、
> うまくいきませんでした。** 束の後ろに置くと、指で挟んでいるように
> 見えません。手前に置くと、腕が体から浮いて「後ろから別の人物が
> 腕を回している」ように見えます（customer-01の試作で実際に起きた）。
> 前後どちらに置いても不自然になるのは、**肩から続く腕**と
> **束を挟む指先**が、体との関係でまったく違う奥行きにあるためです。
> そこで腕を2枚に分け、それぞれを正しい奥行きに置きます。
>
> ```
> 人物（-happy） → 腕（-back-arms） → ブーケ → 指先（-front-hands）
> ```
>
> 描くときの決めごと
>   ・枠は人物と同じ 800×800。同じ位置に重ねるだけで合うこと
>   ・back-arms は、**肩・上腕から自然に続いて見えること**が最優先。
>     左右の外側から差し込まれたように見えてはいけない
>   ・back-arms の角度は「自分の体の前へ、下から出す」形にする。
>     「後ろから回し込む」角度にしない
>   ・back-arms の袖は、`-happy` 本体のブラウスと同じ服として、
>     しわ・ボリューム・袖口の位置がつながって見えること
>   ・front-hands は指先・手首だけ。**小さく**。束を包む手は、
>     束より目立ってはいけない
>   ・front-hands は完全な左右対称にしない。「下からそっと支えている」形
...
> | 4（後日） | `assets/customers/customer-01-back-arms.png` | 肩から続く腕（→ 上の「腕は2枚に分けて持つ」を参照） |
> | 5（後日） | `assets/customers/customer-01-front-hands.png` | 束を挟む指先だけ（同上） |
>
...
> 調整してよい。お渡し画面（`-happy` / `-back-arms` / `-front-hands`）の座標合わせは、
> その2枚を作る段になってから確認する。
