...
- 29種の切り花が、季節をまたいで並びます。花をタップすると、その花だけが前に出ます。
...
```bash
npm run build && npx vite preview --port 4231 &
node tools/qa.mjs 4231      # 通しで遊べるか（7項目）
node tools/weight.mjs       # 画面ごとに何MB読み込むか
```

`qa.mjs` は**押せるはずのものが押せない**穴を探します。
一度「お渡しの画面から出られない」という詰みを出したので、画面を足したら必ず回します。

`weight.mjs` は「扉を押した直後に何MB読むか」を出します。
**開店前の30秒は、このゲームでいちばん待たせてはいけない時間**です。
花や小物を足したら、ここを見てください。
...
public/assets/
  flowers/    29枚          scenes/  窓4枚 + 店内5枚
    small/    29枚（512px・棚とアルバム用。同じ絵の縮小）
